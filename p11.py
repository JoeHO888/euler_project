...
A= np.array([[int(i) for i in e] for e in A])
...
def diagonal(matrix,n):
  ...
  if n>=0:
    for i in range(matrix.shape[0]):
      if i+n < matrix.shape[0]:
        result.append(matrix[i,i+n])
  else:
    for i in range(matrix.shape[0]):
      if i-n< matrix.shape[0]:
        result.append(matrix[i-n,i])
...
first_four=[max_4(A),max_4(reflex(A)),max_4(rot_90(A)),max_4(reflex(rot_90(A)))]

test = [max([max_4(diagonal(A,i)) for i in range(17)]), 
        max([max_4(diagonal(rot_90(A),i)) for i in range(17)]), 
        max([max_4(diagonal(reflex(A),i)) for i in range(17)]),
        max([max_4(reflex(diagonal(A,i))) for i in range(17)])]

t1=time.time()
print(max(test+first_four))
